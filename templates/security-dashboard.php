 ...
 * Pecodex Security �?" Cybersecurity dashboard rendered inside WordPress admin.
...
/* �"?�"? Material Symbols �?" force correct rendering inside WP admin �"?�"? */
...
/* �"?�"? Container grid background (subtle) �"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"? */
.grid-cell::before {
  content: '';
  position: absolute;
  inset: -6px;
  ...
  <div id="ps-main">
    ...
    <header id="ps-header">
      ...
      <div class="header-search">
        ...
        <input type="text" placeholder="Hae järjestelmistä�?�" id="header-search"/>
      </div>
            ...
            <div class="widget-controls-inner" style="height: auto; padding: 18px;">
              ...
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; max-height: 380px; overflow-y: auto; padding-right: 8px;">
                ...
                $modules = [
                  ...
                  ['id' => 'captcha', 'name' => '�"lykäs Captcha', 'icon' => 'fact_check', 'color' => '#f97316'],
  ...
  <div id="threat-modal">
    ...
    <div class="modal-section">
      ...
      <div class="modal-grid">
        <div><span class="lbl">Maa:</span></div><div><span class="val" id="modal-origin">�?"</span></div>
        <div><span class="lbl">IP:</span></div><div><span class="val modal-ip" id="modal-ip">�?"</span></div>
        <div><span class="lbl">Hyökkäys:</span></div><div><span class="val" id="modal-attack">�?"</span></div>
      </div>
    </div>
    <div class="modal-section">
      <div class="modal-section-title">Kohderesurssi</div>
      <div class="modal-grid">
        <div><span class="lbl">Isäntä:</span></div><div><span class="val" id="modal-host">�?"</span></div>
        <div><span class="lbl">Päätepiste:</span></div><div><span class="val" style="font-family:monospace;" id="modal-endpoint">�?"</span></div>
      </div>
    </div>
    <div class="modal-status-row">
      <span id="modal-status" style="color:#dc2626;">�?"</span>
      <span style="color:#dc2626;display:flex;align-items:center;gap:5px;">
        <span style="width:7px;height:7px;border-radius:50%;background:#dc2626;animation:blink 1.2s infinite;display:inline-block;"></span>
...
<div id="lockdown-overlay">
  ...
  <h1>J�"RJESTELM�"N SULKUTILA AKTIIVINEN</h1>
...
/* �"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?
   ...
   Drawn after map is ready so lat/lng �?' px is accurate
�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"?�"? */
...
function openModal(data) {
  ...
  document.getElementById('modal-attack').textContent   = data.attack   || '�?"';
  document.getElementById('modal-host').textContent     = data.host     || '�?"';
...
const TWEAK_DEFS = {
	...
	prevent_enum: { title: "Estä Käyttäjien Luetteleminen", desc: "Estä botteja löytämästä käyttäjätunnuksia.", overview: "Oletuksena hyökkääjät voivat helposti löytää sivustosi käyttäjätunnukset skannaamalla kirjoittaja-arkistoja. Tämä tekee brute-force -hyökkäyksistä paljon helpompia.", fix: "Estämme skriptit, jotka yrittävät automaattisesti luetella käyttäjiäsi." },
	...
	change_admin: { title: "Vaihda 'admin'-käyttäjätunnus", desc: "Varmista, ettei kenelläkään käyttäjällä ole oletus 'admin'-tunnusta.", overview: "Käyttäjätunnus 'admin' on ensimmäinen asia, jota hakkerit yrittävät yrittäessään brute force -hyökkäystä sivustollesi.", fix: "Luo uusi järjestelmänvalvojan tili eri käyttäjätunnuksella, ja poista sitten vanha 'admin'-tili.", toggleable: false, action_text: "Toimenpide vaaditaan WP-Käyttäjissä" },
	login_duration: { title: "Kirjautumisen Kesto", desc: "Vähennä sitä aikaa, jonka käyttäjät pysyvät kirjautuneina.", overview: "Oletuksena käyttäjät, jotka valitsevat 'muista minut', pysyvät kirjautuneina 14 päivää. Tämän vähentäminen rajoittaa ikkunaa, jolloin hyökkääjä voi hyödyntää hylättyä istuntoa.", fix: "Vähennämme oletuskirjautumisen kestoa." },
	...
	prevent_php: { title: "Estä PHP:n Suoritus", desc: "Estä PHP:n suoritus lataushakemistossa.", overview: "Latauskansion pitäisi sisältää vain mediaa. Jos hakkeri lataa tänne haitallisen PHP-skriptin, tämä estää sen suorittamisen.", fix: "Asetamme suojauksia lataushakemistoon estääksemme PHP:n suorittamisen." },
...
window.pmcSec = {
	...
	async fetchAuditLog(page = 1) {
		...
		if (res.success && tbody && res.data && res.data.items) {
			...
			res.data.items.forEach(log => {
				const sevConfig = {
					critical: { label: '�Y"� Kriittinen', color: '#fef2f2', text: '#dc2626' },
					warning:  { label: '�YY� Varoitus',  color: '#fffbeb', text: '#d97706' },
					info:     { label: '�Y"� Info',      color: '#eff6ff', text: '#2563eb' },
				};
			...
			if (tbody && res.data.items) {
				...
				if (res.data.items.length === 0) {
					...
					res.data.items.forEach(log => {
						...
						let actionBtn = `<button class="px-2 py-1 bg-red-50 text-red-700 hover:bg-red-100 border border-red-200 rounded text-[10px] font-bold" onclick="pmcSec.banIp('${ip}')">Estä IP</button>`;
						tbody.innerHTML += `<tr>
	...
	renderBannedIps(ips) {
		...
		if (!tbody) return;
		if (ips.length === 0) {
			tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;color:#6b7280;">Ei estettyjä IP-osoitteita.</td></tr>';
			return;
		}
		let html = '';
		ips.forEach(ip => {
...
window.pmcSaveSubscribers = async (btn) => {
	...
	if (res.success) {
		...
		alert('Ilmoitusasetusten tallennus ep�onnistui: ' + (res.data || 'Tuntematon virhe'));
	}
...
window.pmcSendTestNotifications = async (btn) => {
	btn.disabled = true;
	const oldText = btn.textContent;
	btn.textContent = 'L�hetet��n...';

	const res = await pmcSec.post('pmc_security_send_test_notifications', {});
	if (res.success) {
		alert('Testis�hk�postit l�hetetty onnistuneesti kaikille tilaajille!');
	} else {
		alert('Testis�hk�postien l�hetys ep�onnistui: ' + (res.data || 'Tuntematon virhe'));
	}
